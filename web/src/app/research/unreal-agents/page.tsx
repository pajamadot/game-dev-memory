...
import { runUnrealAgentsDigest } from "../../actions";
...
async function loadMarkdown(): Promise<string> {
  const mdPath = path.join(process.cwd(), "src", "content", "research", "unreal-agents.md");
...
export default async function UnrealAgentsResearchPage() {
  ...
  if (userId) {
    try {
      const res = await apiJson<{ digests: Digest[] }>("/api/research/unreal-agents/digests?limit=10");
      digests = res.digests || [];
    } catch (e) {
      ...
      <div className="mx-auto w-full max-w-5xl px-6 py-10">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-baseline sm:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-zinc-950">Unreal Agents</h1>
            <p className="mt-1 text-sm text-zinc-600">Using Unreal Engine + AI agents to build games, backed by project memory.</p>
          </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-zinc-600">
                    Generated by the API worker cron and stored as `research` memories tagged `unreal-agents`.
                  </p>
                </div>
                {userId ? (
                  <form action={runUnrealAgentsDigest}>

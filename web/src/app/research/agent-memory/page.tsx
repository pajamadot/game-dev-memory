...
import { runAgentMemoryDigest } from "../../actions";
...
export default async function AgentMemoryResearchPage() {
  ...
  if (userId) {
    try {
      const res = await apiJson<{ digests: Digest[] }>("/api/research/agent-memory/digests?limit=10");
      digests = res.digests || [];
    } catch (e) {
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-zinc-600">
                    Generated by the API worker cron and stored as `research` memories tagged `agent-memory`.
                  </p>
                </div>
                {userId ? (
                  <form action={runAgentMemoryDigest}>

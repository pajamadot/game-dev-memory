...
async function loadMarkdown(): Promise<string> {
  const mdPath = path.join(process.cwd(), "src", "content", "research", "pageindex.md");
  return await readFile(mdPath, "utf8");
}

export default async function PageIndexResearchPage() {
      ...
      <div className="mx-auto w-full max-w-5xl px-6 py-10">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-baseline sm:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-semibold tracking-tight text-zinc-950">PageIndex</h1>
            <p className="mt-1 text-sm text-zinc-600">Reasoning-style document indexing for long specs and manuals.</p>
          </div>

...
import { createProAgentSession } from "./actions";
...
export default async function ProAgentSessionsPage() {
      ...
      <div className="min-h-screen bg-[radial-gradient(900px_circle_at_10%_-20%,#bfdbfe_0%,transparent_55%),radial-gradient(900px_circle_at_90%_0%,#fde68a_0%,transparent_55%),linear-gradient(180deg,#fafafa_0%,#ffffff_60%,#fafafa_100%)]">
        <div className="mx-auto flex min-h-screen w-full max-w-3xl flex-col justify-center px-6 py-16">
          <div className="rounded-3xl border border-zinc-200/70 bg-white/70 p-10 shadow-sm backdrop-blur">
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-600">Sign in to create and continue pro agent sessions.</p>
    ...
    <div className="min-h-screen bg-[radial-gradient(900px_circle_at_10%_-20%,#bfdbfe_0%,transparent_55%),radial-gradient(900px_circle_at_90%_0%,#fde68a_0%,transparent_55%),linear-gradient(180deg,#fafafa_0%,#ffffff_60%,#fafafa_100%)]">
      <div className="mx-auto w-full max-w-6xl px-6 py-10">
        <header className="flex flex-col gap-3 sm:flex-row sm:items-baseline sm:justify-between">
          <div>
            <p className="text-xs font-semibold tracking-[0.22em] text-zinc-500">AGENT PRO</p>
            <h1 className="text-2xl font-semibold tracking-tight text-zinc-950">Pro Agent Sessions</h1>
        ...
        <main className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-12">
          <section className="lg:col-span-4">
            <div className="rounded-3xl border border-zinc-200/70 bg-white/70 p-6 shadow-sm backdrop-blur">
              <h2 className="text-sm font-semibold tracking-wide text-zinc-900">New pro session</h2>
              <p className="mt-1 text-xs leading-5 text-zinc-600">
                Sessions are project-scoped. Use org scope for shared sessions; personal scope stays private.
              </p>

              <form action={createProAgentSession} className="mt-4 space-y-3">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-700" htmlFor="pro-agent-session-project">
                    Project
                  </label>
                  <select
                    id="pro-agent-session-project"
                    name="project_id"
                    required
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-700" htmlFor="pro-agent-session-title">
                    Title (optional)
                  </label>
                  <input
                    id="pro-agent-session-title"
                    name="title"
          ...
          <section className="lg:col-span-8">
            <div className="rounded-3xl border border-zinc-200/70 bg-white/70 p-6 shadow-sm backdrop-blur">
              ...
              {sessions.length === 0 ? (
                <p className="mt-4 text-sm text-zinc-600">No pro agent sessions yet.</p>
              ) : (

...
// "New projects" radar: product/repo launches and agentic dev signals.
const FEEDS: FeedSource[] = [
  { name: "GitHub Blog", url: "https://github.blog/feed/" },
  { name: "Hacker News (game dev projects)", url: 'https://hnrss.org/newest?q=%22game%20dev%22%20open%20source' },
  { name: "Hacker News (agent tooling projects)", url: 'https://hnrss.org/newest?q=%22AI%20agent%22%20open%20source' },
  {
    name: "arXiv cs.SE/cs.AI (agentic software projects)",
    url: "https://export.arxiv.org/api/query?search_query=(cat:cs.SE+OR+cat:cs.AI)+AND+(all:agent+OR+all:agentic)+AND+(all:software+OR+all:developer)&start=0&max_results=20",
  },
];
...
async function fetchFeed(source: FeedSource): Promise<FeedItem[]> {
  const res = await fetch(source.url, {
    headers: {
      "user-agent": "game-dev-memory/0.1 (new-projects-research; +https://pajamadot.com)",
...
function renderDigestMarkdown(opts: { date: string; items: FeedItem[]; sources: FeedSource[] }): string {
  const lines: string[] = [];
  lines.push(`# New Projects Daily Digest (${opts.date})`);
  lines.push("");
  lines.push("Automated daily scan for new projects, tools, and launches relevant to game-dev memory workflows.");
  lines.push("");
  ...
  lines.push("- Use the curated /research/new-projects page for stable implementation guidance.");
  return lines.join("\n");
}
...
async function upsertDailyDigestMemory(
  ...
  const title = `New Projects Daily Digest ${opts.date}`;
  ...
  const id = crypto.randomUUID();
  const tags = ["research", "new-projects", "daily", opts.date];
  const context = {
    kind: "new-projects-digest",
    date: opts.date,
  ...
  await db.query(
    ...
    [
      ...
      opts.tenant.tenant_id,
      opts.projectId,
      `Daily research digest: New Projects (${opts.date})`,
      JSON.stringify({ kind: "new-projects-digest", date: opts.date, item_count: opts.feedItems.length }),
      opts.nowIso,
      opts.actorId,
    ]
  );
...
export async function runNewProjectsDailyDigestForTenant(env: Env, tenant: Tenant, actorId: string, date: Date) {
...
export async function runNewProjectsDailyDigestForAllTenants(
  env: Env,
  date: Date
